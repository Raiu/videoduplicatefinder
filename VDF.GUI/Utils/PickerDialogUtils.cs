...
internal static class PickerDialogUtils
{
	...
	{
		var paths = await ApplicationHelpers.MainWindow.StorageProvider.OpenFilePickerAsync(
			options
		);

		if (paths != null && paths.Count > 0 && paths[0].TryGetLocalPath() is string uriPath)
			return uriPath;
	...
	{
		var paths = await ApplicationHelpers.MainWindow.StorageProvider.OpenFolderPickerAsync(
			options
		);
