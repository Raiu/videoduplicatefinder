...
internal class DatabaseViewerVM : ReactiveObject
{
	...
	public DatabaseViewerVM(DataGrid dataGrid)
	{
		...
		DatabaseFiles = JsonSerializer.Deserialize<ObservableCollection<FileEntry>>(
			File.ReadAllBytes(TempDatabaseFile),
			serializerOptions
		)!;
	...
	private void GetDataGrid_RowEditEnded(object? sender, DataGridRowEditEndedEventArgs e) =>
		canDeleteRows = true;

	private void GetDataGrid_CellEditEnded(object? sender, DataGridCellEditEndedEventArgs e) =>
		canDeleteRows = true;

	private void GetDataGrid_BeginningEdit(object? sender, DataGridBeginningEditEventArgs e) =>
		canDeleteRows = false;
	...
	public void Save()
	{
		File.WriteAllBytes(
			TempDatabaseFile,
			JsonSerializer.SerializeToUtf8Bytes(DatabaseFiles, serializerOptions)
		);
		...
		catch (Exception e)
		{
			Logger.Instance.Info(
				$"Failed to delete temporarily database file '{TempDatabaseFile}', because of {e}"
			);
		}
	}

	public ReactiveCommand<Unit, Unit> DeleteSelectedEntries =>
