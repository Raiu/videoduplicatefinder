...
#pragma warning restore CA1822 // Mark members as static
	...
	static readonly List<string> _CustomCommandList = typeof(SettingsFile.CustomActionCommands)
		.GetProperties(BindingFlags.Public | BindingFlags.Instance)
		.Select(p => p.Name)
		.ToList();
	public List<string> CustomCommandList => _CustomCommandList;
	PropertyInfo _SelectedCustomCommand = typeof(SettingsFile.CustomActionCommands).GetProperty(
		_CustomCommandList[0]
	)!;
	...
	public ReactiveCommand<Unit, Unit> OpenHWInfoLinkCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			...
			catch
			{
				await MessageBoxService.Show(
					"Failed to open URL: https://trac.ffmpeg.org/wiki/HWAccelIntro#PlatformAPIAvailability"
				);
			}
		});
	public ReactiveCommand<Unit, Unit> AddIncludesToListCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			var result = await Utils.PickerDialogUtils.OpenDialogPicker(
				new FolderPickerOpenOptions() { AllowMultiple = true, Title = "Select folder" }
			);
	...
	public ReactiveCommand<Unit, Unit> AddFilePathContainsTextToListCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
	...
	public ReactiveCommand<Unit, Unit> AddFilePathNotContainsTextToListCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			var result = await InputBoxService.Show("New Entry");
			if (string.IsNullOrEmpty(result))
	...
	public ReactiveCommand<ListBox, Action> RemoveFilePathNotContainsTextFromListCommand =>
		...
		{
			while (lbox.SelectedItems?.Count > 0)
				SettingsFile.Instance.FilePathNotContainsTexts.Remove(
					(string)lbox.SelectedItems[0]!
				);
			return null!;
		});

	public ReactiveCommand<ListBox, Action> RemoveIncludesFromListCommand =>
		ReactiveCommand.Create<ListBox, Action>(lbox =>
		{
			while (lbox.SelectedItems?.Count > 0)
	...
	public ReactiveCommand<Unit, Unit> ClearIncludesListCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			var result = await MessageBoxService.Show(
				"Are you sure you want to clear the list of ALL included folders?",
				MessageBoxButtons.Yes | MessageBoxButtons.Cancel
			);
			if (result == MessageBoxButtons.Yes)
				SettingsFile.Instance.Includes.Clear();
		});

	public ReactiveCommand<Unit, Unit> ClearBlacklistListCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			var result = await MessageBoxService.Show(
				"Are you sure you want to clear the list of ALL excluded folders?",
				MessageBoxButtons.Yes | MessageBoxButtons.Cancel
			);
			if (result == MessageBoxButtons.Yes)
	...
	public ReactiveCommand<Unit, Unit> AddBlacklistToListCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			var result = await Utils.PickerDialogUtils.OpenDialogPicker(
				new FolderPickerOpenOptions() { AllowMultiple = true, Title = "Select folder" }
	...
	public ReactiveCommand<ListBox, Action> RemoveBlacklistFromListCommand =>
		ReactiveCommand.Create<ListBox, Action>(lbox =>
		{
			while (lbox.SelectedItems?.Count > 0)
	...
	public ReactiveCommand<Unit, Unit> ClearLogCommand =>
		ReactiveCommand.Create(() =>
		{
			LogItems.Clear();
		});
	public ReactiveCommand<Unit, Unit> SaveLogCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			var result = await Utils.PickerDialogUtils.SaveFilePicker(
				new FilePickerSaveOptions() { DefaultExtension = ".txt" }
			);
			if (string.IsNullOrEmpty(result))
				return;
	...
	public ReactiveCommand<Unit, Unit> SaveSettingsProfileCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			var result = await Utils.PickerDialogUtils.SaveFilePicker(
				new FilePickerSaveOptions()
				{
					...
					{
						new FilePickerFileType("Setting File")
						{
	...
	public ReactiveCommand<Unit, Unit> LoadSettingsProfileCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			...
			{
				await MessageBoxService.Show(
					$"Loading settings from file has failed: {ex.Message}"
				);
				return;
			}
