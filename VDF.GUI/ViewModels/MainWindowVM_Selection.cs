...
{
	public ReactiveCommand<Unit, Unit> OpenCustomSelectionCommand =>
	...
	public ReactiveCommand<Unit, Unit> CheckCustomCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			ExpressionBuilder dlg = new();
			((ExpressionBuilderVM)dlg.DataContext!).ExpressionText = SettingsFile
				.Instance
			...
			SettingsFile.Instance.LastCustomSelectExpression = (
				(ExpressionBuilderVM)dlg.DataContext
			).ExpressionText;
			...
			{
				if (blackListGroupID.Contains(first.ItemInfo.GroupId))
					continue; //Dup has been handled already

				IEnumerable<DuplicateItemVM> l = Duplicates.Where(a =>
					a.IsVisibleInFilter && a.ItemInfo.GroupId == first.ItemInfo.GroupId
				);
				IEnumerable<DuplicateItemVM> matches;

				try
				{
					var interpreter = new Interpreter().ParseAsDelegate<Func<DuplicateItem, bool>>(
						SettingsFile.Instance.LastCustomSelectExpression,
						shortIdentifier
					);
				...
				catch (ParseException e)
				{
					await MessageBoxService.Show(
						$"Failed to parse '{SettingsFile.Instance.LastCustomSelectExpression}': {e}"
					);
					return;
				}
				...
				if (matches.Count() == l.Count())
				{
					if (!userAsked)
					{
						MessageBoxButtons? result = await MessageBoxService.Show(
							$"There are groups where all items match your expression, for example '{first.ItemInfo.Path}'.{Environment.NewLine}{Environment.NewLine}Do you want to have all items checked (Yes)? Or do you want to have NO items in these groups checked (No)?",
	...
	public ReactiveCommand<Unit, Unit> CheckWhenIdenticalCommand =>
		ReactiveCommand.Create(() =>
		{
			HashSet<Guid> blackListGroupID = new();
			...
			foreach (var first in Duplicates)
			{
				...
				var l = Duplicates.Where(d =>
					d.IsVisibleInFilter
					&& d.EqualsFull(first)
					&& !d.ItemInfo.Path.Equals(first.ItemInfo.Path)
				);
	...
	public ReactiveCommand<Unit, Unit> CheckWhenIdenticalButSizeCommand =>
		ReactiveCommand.Create(() =>
		{
			HashSet<Guid> blackListGroupID = new();
	...
	public ReactiveCommand<Unit, Unit> CheckOldestCommand =>
		ReactiveCommand.Create(() =>
		{
			HashSet<Guid> blackListGroupID = new();
	...
	public ReactiveCommand<Unit, Unit> CheckNewestCommand =>
		ReactiveCommand.Create(() =>
		{
			HashSet<Guid> blackListGroupID = new();
	...
	public ReactiveCommand<Unit, Unit> CheckLowestQualityCommand =>
		ReactiveCommand.Create(() =>
		{
			HashSet<Guid> blackListGroupID = new();
			...
			foreach (var first in Duplicates)
			{
				if (blackListGroupID.Contains(first.ItemInfo.GroupId))
					continue; //Dup has been handled already
				IEnumerable<DuplicateItemVM> l = Duplicates.Where(d =>
					d.IsVisibleInFilter
					&& d.EqualsButQuality(first)
					&& !d.ItemInfo.Path.Equals(first.ItemInfo.Path)
				);
				var dupMods = l as List<DuplicateItemVM> ?? l.ToList();
				if (!dupMods.Any())
				...
				if (
					keep.ItemInfo.Path.Equals(dupMods[0].ItemInfo.Path)
					|| dupMods.Count(d => d.ItemInfo.Duration == keep.ItemInfo.Duration) > 1
				)
				...
				if (
					!keep.ItemInfo.IsImage
					&& (
						keep.ItemInfo.Path.Equals(dupMods[0].ItemInfo.Path)
						|| dupMods.Count(d => d.ItemInfo.FrameSizeInt == keep.ItemInfo.FrameSizeInt)
							> 1
					)
				)
				...
				if (
					...
					&& (
						keep.ItemInfo.Path.Equals(dupMods[0].ItemInfo.Path)
						|| dupMods.Count(d => d.ItemInfo.Fps == keep.ItemInfo.Fps) > 1
					)
				)
	...
	public ReactiveCommand<Unit, Unit> DeleteHighlitedCommand =>
		ReactiveCommand.Create(() =>
		{
	...
	public ReactiveCommand<Unit, Unit> DeleteSelectionWithPromptCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			MessageBoxButtons? dlgResult = await MessageBoxService.Show(
				"Delete files also from DISK?",
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
	...
	public ReactiveCommand<Unit, Unit> DeleteSelectionCommand =>
		ReactiveCommand.Create(() =>
		{
			Dispatcher.UIThread.InvokeAsync(() =>
			{
	...
	public ReactiveCommand<Unit, Unit> DeleteSelectionPermanentlyCommand =>
		ReactiveCommand.Create(() =>
		{
			Dispatcher.UIThread.InvokeAsync(() =>
			{
	...
	public ReactiveCommand<Unit, Unit> RemoveSelectionFromListCommand =>
		ReactiveCommand.Create(() =>
		{
			Dispatcher.UIThread.InvokeAsync(() =>
			{
	...
	public ReactiveCommand<Unit, Unit> RemoveSelectionFromListAndBlacklistCommand =>
		ReactiveCommand.Create(() =>
		{
			Dispatcher.UIThread.InvokeAsync(() =>
			{
	...
	public ReactiveCommand<Unit, Unit> CreateSymbolLinksForSelectedItemsCommand =>
		ReactiveCommand.Create(() =>
		{
			Dispatcher.UIThread.InvokeAsync(() =>
			{
	...
	public ReactiveCommand<Unit, Unit> CreateSymbolLinksForSelectedItemsAndBlacklistCommand =>
		ReactiveCommand.Create(() =>
		{
			Dispatcher.UIThread.InvokeAsync(() =>
			{
	...
	public ReactiveCommand<Unit, Unit> CopySelectionCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			var result = await Utils.PickerDialogUtils.OpenDialogPicker(
				new FolderPickerOpenOptions() { Title = "Select folder" }
			);

			if (result == null || result.Count == 0)
				return;

			Utils.FileUtils.CopyFile(
				Duplicates.Where(s => s.Checked),
				result[0],
				true,
				false,
	...
	public ReactiveCommand<Unit, Unit> MoveSelectionCommand =>
		ReactiveCommand.CreateFromTask(async () =>
		{
			var result = await Utils.PickerDialogUtils.OpenDialogPicker(
				new FolderPickerOpenOptions() { Title = "Select folder" }
	...
	internal void RunCustomSelection(CustomSelectionData data)
	{
		...
		if (data.IgnoreGroupsWithSelectedItems)
		{
			...
			{
				if (blackList.Contains(first.ItemInfo.GroupId))
					continue;
