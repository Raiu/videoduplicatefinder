...
static class FFProbeJsonReader
{
	...
	static ReadOnlySpan<byte> StreamsKeyword =>
		new byte[] { 0x73, 0x74, 0x72, 0x65, 0x61, 0x6D, 0x73 }; // = streams
	...
	public static MediaInfo Read(byte[] data, string file)
	{
				...
				case JsonTokenType.Number:
					if (!json.TryGetInt32(out int valueInteger))
					{
						...
						System.Diagnostics.Trace.TraceWarning(
							$"JSON number parse error: \"{lastKey}\" = {System.Text.Encoding.UTF8.GetString(valueSpan.ToArray())}, file = {file}"
						);
#endif
						break;
					}
		...
		var info = new MediaInfo { Streams = new MediaInfo.StreamInfo[streams.Count] };

		if (
			format.ContainsKey("duration")
			&& TimeSpan.TryParse((string)format["duration"], out var duration)
		)
			/*
		...
		for (int i = 0; i < streams.Count; i++)
		{
			...
			if (
				streams[i].ContainsKey("bit_rate")
				&& long.TryParse((string)streams[i]["bit_rate"], out var bitrate)
			)
			{
				foundBitRate = true;
			...
			if (
				streams[i].ContainsKey("sample_rate")
				&& int.TryParse((string)streams[i]["sample_rate"], out var sample_rate)
			)
			...
			if (streams[i].ContainsKey("r_frame_rate"))
			{
				...
				if (stringFrameRate.Contains('/'))
				{
					...
					if (
						split.Length == 2
						&& int.TryParse(split[0], out var firstRate)
						&& int.TryParse(split[1], out var secondRate)
					)
						info.Streams[i].FrameRate =
							(firstRate > 0 && secondRate > 0) ? firstRate / (float)secondRate : -1f;
				}
			}
		}
		...
		if (
			...
			&& format.ContainsKey("bit_rate")
			&& long.TryParse((string)format["bit_rate"], out var formatBitrate)
		)
