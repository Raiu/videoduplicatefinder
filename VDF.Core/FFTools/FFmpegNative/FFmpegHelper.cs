	...
	private static bool FindFFmpegLibraryFiles()
	{
		try
		{
			...
			if (path != null)
			{
				...
				{
					if (
						CheckForFfmpegLibraryFilesInFolder(
							Path.Combine(
			...
			{
				string firstLibrary =
					$"lib{ffmpeg.LibraryVersionMap.Keys.First()}.so.{ffmpeg.LibraryVersionMap.Values.First()}";
				foreach (
					var libDir in Directory.EnumerateDirectories(
						"/usr/",
						"lib*",
