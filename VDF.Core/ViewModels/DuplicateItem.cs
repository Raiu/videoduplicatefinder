	...
	public DuplicateItem(FileEntry file, float difference, Guid groupID, DuplicateFlags flags)
	{
		...
		if (!file.IsImage && file.mediaInfo?.Streams?.Length > 0)
		{
			...
			*/
			...
			{
				if (
					file.mediaInfo.Streams[i]
						.CodecType?.Equals("video", StringComparison.OrdinalIgnoreCase) == true
					&& file.mediaInfo.Streams[i].Width * file.mediaInfo.Streams[i].Height
						>= selVideo[1]
				)
				{
					selVideo[0] = i;
					...
					file.mediaInfo.Streams[i]
						.CodecType?.Equals("audio", StringComparison.OrdinalIgnoreCase) == true
			...
			if (selVideo[0] >= 0)
			{
				...
				FrameSize =
					file.mediaInfo.Streams[i].Width + "x" + file.mediaInfo.Streams[i].Height;
		...
		else
		{
			...
			{
				FrameSize =
					file.mediaInfo.Streams[0].Width + "x" + file.mediaInfo.Streams[0].Height;
